...
class DrmPreview(NullPreview):
    ...
    def __init__(self, x=0, y=0, width=1920, height=1080, transform=None):
    ...
    def render_drm(self, picam2a, picam2b, completed_request, completed_request2):
        if completed_request is not None:
            ...
            display_stream_name = picam2a.display_stream_name
            stream = picam2a.stream_map[display_stream_name]

        if completed_request2 is not None:
            display_stream_name = completed_request2.config['display']
            stream2 = completed_request2.stream_map[display_stream_name]
        else:
            display_stream_name = picam2b.display_stream_name
            stream2 = picam2b.stream_map[display_stream_name]
        
        cfg = stream.configuration
        pixel_format = str(cfg.pixel_format)
        width, height = (cfg.size.width, cfg.size.height)

        cfg2 = stream2.configuration
        pixel_format2 = str(cfg2.pixel_format)
        width2, height2 = (cfg2.size.width, cfg2.size.height)
        ...
        if self.plane2 is None:
            if pixel_format not in self.FMT_MAP:
                raise RuntimeError(f"Format {pixel_format} not supported by DRM preview")
            fmt = self.FMT_MAP[pixel_format]

            self.plane2 = self.resman.reserve_overlay_plane(self.crtc, fmt)
            if self.plane2 is None:
                raise RuntimeError("Failed to reserve DRM plane")
            drm_rotation = 1
            if self.transform.hflip:
            ...
            if self.transform.vflip:
                drm_rotation |= 32
            self.plane2.set_prop("rotation", drm_rotation)
            # The second plane we ask for will go on top of the first.
            self.overlay_plane = self.resman.reserve_overlay_plane(self.crtc, pykms.PixelFormat.ABGR8888)
            if self.overlay_plane is None:
                raise RuntimeError("Failed to reserve DRM overlay plane")
        ...
        if completed_request is not None:
            ...
            if fb not in self.drmfbs:
                if self.stop_count != picam2a.stop_count:
                    old_drmfbs = self.drmfbs  # hang on to these until after a new one is sent
                    self.drmfbs = {}
                    self.stop_count = picam2a.stop_count
                fmt = self.FMT_MAP[pixel_format]
        ...
        if completed_request2 is not None:
            fb2 = completed_request2.request.buffers[stream2]

            if pixel_format == "MJPEG":
                img = completed_request2.make_array(display_stream_name).tobytes()
                self.mem.seek(0)
                self.mem.write(img)
                fd = self.fd
                stride = width * 3
            ...
            if fb2 not in self.drmfbs:
                if self.stop_count != picam2b.stop_count:
                    old_drmfbs = self.drmfbs  # hang on to these until after a new one is sent
                    self.drmfbs = {}
                    self.stop_count = picam2b.stop_count
                fmt = self.FMT_MAP[pixel_format]
            ...
            #                        "CRTC_X": x, "CRTC_Y": y, "CRTC_W": 960, "CRTC_H": 1080})
            # ctx.add(self.plane2, {"FB_ID": drmfb2.id, "CRTC_ID": self.crtc.id,
            #                        "SRC_W": width << 16, "SRC_H": height << 16,
            #                        "CRTC_X": 960, "CRTC_Y": y, "CRTC_W": 960, "CRTC_H": 1080})
            # ctx.commit()
